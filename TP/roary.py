...
class Roarier:
    def __init__(self, kingdom, genus, working_dir=os.getcwd()) -> None:
        ...
        self.roary_bin = f"docker run --user {os.getuid()}:{os.getgid()} --rm -it -v {self.gffs_folder}:/data sangerpathogens/roary roary"

    def run_prokka(self) -> None:
        genomes = glob.glob(os.path.join(self.working_dir, "*.fna"))
        for g in genomes:
            locustag = Path(Path(g).stem).stem
            PROKKA_params = f"--kingdom {self.kingdom} --outdir /data/output/PROKKA_{locustag} --genus {self.genus} --locustag {locustag} {os.path.basename(g)}"
            subprocess.run(f"{self.PROKKA_bin} {PROKKA_params}",
                           shell=True, stdout=sys.stderr)
            PROKKA_out_folder = os.path.join(
                self.out_dir, f"PROKKA_{locustag}")
    ...
    def run_roary(self) -> None:
        ...
        for i in range(0, len(gff_list)):
            ...
            gff_list[i] = os.path.join("/data", gff_list[i])
        roary_out = "roary"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to execute the roary pipeline")
    parser.add_argument(
        "working_dir", help="path of containing the .gff files", type=str)
